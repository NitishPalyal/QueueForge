...
export const useJobsList = (page: number, limit: number, statusFilter: JobStatus | 'all', queueFilter: QueueName | 'all') => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let resData: { totalJobs: number; jobs: any[] } = { totalJobs: 0, jobs: [] };
      if (statusFilter !== 'all' && queueFilter !== 'all' && statusFilter !== 'active' && statusFilter !== 'delayed') {
        resData = await jobsApi.getJobsByQueueAndStatus(queueFilter, statusFilter);
      } else if (queueFilter !== 'all') {
        resData = await jobsApi.getJobsByQueue(queueFilter);
      } else if (statusFilter !== 'all' && statusFilter !== 'active' && statusFilter !== 'delayed') {
        ...
        if (queueFilter !== 'all') filtered = filtered.filter((j) => (j.queue_name || j.queueName) === queueFilter);
        return {
          limit,
          page: 1,
...
export const useCreateEmailJob = () => {
  return useMutation({
    mutationFn: (payload: CreateEmailJobPayload) => jobsApi.createEmailJob(payload),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['jobs'] });
      toast.success('Email Job Queued', {
        description: `Job ID: ${data.data?.jobId || 'created'} is now processing`,
      });
    },
    onError: (err: any) => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success('AI Response Job Queued', {
        description: `Job ID: ${data.data?.jobId || 'created'} is pending execution`,
      });
    },
    onError: (err: any) => {
...
export const useCreateImageJob = () => {
  return useMutation({
    mutationFn: (params: CreateImageJobParams) => jobsApi.createImageJob(params),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['jobs'] });
      toast.success('Image Job Queued', {
        description: `Job ID: ${data.data?.jobId || 'created'} added for optimization`,
      });
    },
    onError: (err: any) => {
      toast.error('Failed to Queue Image Job', {
        description: extractErrorMessage(err, 'Failed to queue image processing job'),
      });
    },
  });
};
...
export const useRetryJob = () => {
  return useMutation({
    mutationFn: ({ id, queue }: { id: string; queue: QueueName }) => jobsApi.retryJob(id, queue),
    onSuccess: (retriedJob) => {
      queryClient.invalidateQueries({ queryKey: ['jobs'] });
      if (retriedJob.id) {
        queryClient.invalidateQueries({ queryKey: ['job', 'detail', retriedJob.id] });
      }
      toast.success('Job Retried', { description: `Job ${retriedJob.id} re-queued successfully.` });
    },
    onError: (err: any) => {
...
export const useDeleteJob = () => {
  return useMutation({
    mutationFn: ({ queue, id }: { queue: QueueName; id: string }) => jobsApi.deleteJob(queue, id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['jobs'] });
      toast.success('Job Deleted', { description: 'Job record and related assets removed.' });
    },
    onError: (err: any) => {
