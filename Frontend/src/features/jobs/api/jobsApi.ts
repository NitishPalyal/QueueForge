...
export const jobsApi = {
  ...
  // Create Image Job: backend reads file from multipart form and idempotency_key + priority from headers.
  createImageJob: async ({
    imageFile,
    idempotency_key,
    priority,
  }: CreateImageJobParams) => {
    const formData = new FormData();
    formData.append("image", imageFile, imageFile.name);

    const response = await apiClient.post<APIResponse>(
      "/api/job/imageProcessing",
      formData,
      {
        headers: {
          idempotency_key,
  ...
  getAllJobs: async (page = 1, limit = 50): Promise<PaginatedJobsData> => {
    const response = await apiClient.get<APIResponse<PaginatedJobsData>>(
      "/api/job/getAllJobs",
      {
        params: { page, limit },
      },
  ...
  getJobById: async (id: string): Promise<JobDetailResponse> => {
    const response = await apiClient.get<APIResponse<JobDetailResponse>>(
      `/api/job/getJob/${id}`,
    );
    if (response.data.data) {
      return response.data.data;
