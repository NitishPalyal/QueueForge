import { flowProducer } from "./batchJob.producer.js";
import * as BatchJobRepo from "./batchJob.repository.js";
import * as JobRepo from "../job/job.repository.js";
import { ImageJobPayloadSchema } from "../shared/zod.schema.js";
import { logger } from "../shared/logger.js";
import { QUEUE_BY_TYPE, } from "./batchJob.types.js";
import { createAiResponseJobService, createImageProcessingJobService, createMailJobService, deleteImageJobUploadedAndProcessedImageService, getImageJobUploadedAndProcessedImageUrlService, } from "../job/job.service.js";
function buildFlowTreeService({ steps, jobs, batchId, }) {
    try {
        const toFlowJob = ({ step, job, isLastStep }) => {
        ...
        return steps.slice(1).reduce((childNode, step, i) => ({
            ...toFlowJob({
                step,
                job: jobs[i + 1],
...
export async function createBatchService({ steps, }) {
    try {
        const data = {
        ...
        await flowProducer.add(buildFlowTreeService({ steps, jobs, batchId: batch.id }));
        return batch;
    }
    catch (error) {
...
export async function setBatchStatusCompletedService({ dbJobId, batchId, isLastStep, }) {
    try {
...
export async function getBatchJobsService(batchId) {
    try {
        const batchJobs = await JobRepo.findByBatch(batchId);
        return await Promise.all(batchJobs.map(async (job) => {
            if (job.type !== "image") {
                return job;
            }
...
export async function getAllBatchesService({ limit, page, }) {
    try {
