...
import { MailSchema } from "../../shared/zod.schema.js";
import { logger } from "../../shared/logger.js";
import { addJobInMailQueueService } from "../mail/mail.service.js";
    ...
    for (const provider of providers) {
        ...
        for (const model of provider.models) {
            try {
                ...
                logger.warn(`Failed: ${provider.name}/${model.name}`, "ai.service", error);
            }
        }
    }
...
export async function generateMailContentService({ to, prompt, }) {
    try {
        const aiPrompt = `
...
export async function generateAiResponseService({ prompt, jobId, }) {
    try {
...
export async function generateAiResponseForEmailService({ prompt, to, jobId, batchId, isLastStep, }) {
    try {
        ...
        logger.error("Error in generateAiResponseForEmailService", "ai.service", error);
        throw error;
    }
}
// ADD JOB IN AI QUEUE//
